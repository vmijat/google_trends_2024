...
#' https://trends.withgoogle.com/year-in-search/2024/us/ and place them in /data

# Read downloaded files
folder <- file.path("data", "us")
...
# Annotations
plot_titles <- list(
  title = glue("Google Trends <span style='color:{google_colors[1]}'>2024</span>"),
  subtitle = "Weekly relative search volume of the top search terms in the U.S.
  (categories \"Searches\" and \"News\"). 
  The search volume is scaled to a value range from 0 to 100
  (= maximum search interest per search term).",
  caption = "Data: **Google Trends** (web search Germany) | 
...
ggsave(file.path("plots", "gtrends_us_2024_dark.png"), device = ragg::agg_png,
...
ggsave(file.path("plots", "gtrends_us_2024_light.png"), device = ragg::agg_png,
       dpi = 300, width = 6, height = 6)
